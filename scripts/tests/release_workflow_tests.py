...
class CrossPlatformReleaseWorkflowTests(unittest.TestCase):
    ...
    def test_prepare_appends_download_and_unsigned_notes(self):
        append = find_step(
            self.jobs["prepare"], "Append download and signing notes to body"
        )
        run = append["run"]
        for name in (
            "windows_x64",
            ...
            self.assertIn(name, run, msg=f"download list missing {name}")
