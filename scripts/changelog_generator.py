...
- 条目：提交标题原文 + 末尾括号组 `([#N](url) [@user](url))`，无 PR 时仅 `([@user](url))`
- 贡献者：通过 GitHub API 把 git 提交解析成 @登录名，逐条全署并带 profile 链接，
  结尾再汇总去重后的 `## Thanks to` 列表
...
def main() -> None:
    ...
    lines = [f"## {version} - {release_date(args.tag)}", ""]
    contributors: list[str] = []
    seen_contributor: set[str] = set()
    for category, title in CATEGORY_ORDER:
        ...
        lines.append("")
        for commit in items:
            logins = resolve_logins(args.repo, commit["hash"], commit["authors"])
            for login in logins:
                key = login.lower()
                if key not in seen_contributor:
                    seen_contributor.add(key)
                    contributors.append(login)
            mentions = " ".join(link_login(login) for login in logins)
            if commit["number"]:
                pr = f"[#{commit['number']}](https://github.com/{args.repo}/pull/{commit['number']})"
    ...
    if contributors:
        ...
        for login in contributors:
            lines.append(f"- {link_login(login)}")
        lines.append("")
    if base:
        lines.append(
            f"**Full Changelog**: [{base}...{args.tag}]"
            f"(https://github.com/{args.repo}/compare/{base}...{args.tag})"
        )
        lines.append("")
