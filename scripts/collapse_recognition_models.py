...
def collapse_svm(model) -> SvmModel:
...
def collapse_knn(model) -> KnnModel:
    return {
...
def verify_svm(source, collapsed: SvmModel) -> None:
    rng = np.random.default_rng(0)
    probes = rng.uniform(0, 1, (10_000, collapsed["w"].shape[0]))
    expected = source.predict(probes).astype(bool)
    actual = probes @ collapsed["w"] + collapsed["b"] > 0
    np.testing.assert_array_equal(actual, expected)


def verify_knn(source, collapsed: KnnModel) -> None:
    rng = np.random.default_rng(0)
    ...
    actual = np.asarray(
        [
            collapsed["classes"][
                collapsed["y"][np.argmin(np.sum((collapsed["X"] - probe) ** 2, axis=1))]
            ]
            for probe in probes
        ]
    )
