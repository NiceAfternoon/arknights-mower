...
- 重写后无 import 即不会被 PyInstaller 打包；可选防御：在两 spec 的 `mower_a` Analysis `excludes` 追加 `"sklearn", "scikit_image", "scipy"`（非必须）。
...
4. `depotREC.py:11,43-51` 改 `vision_np.hog`；`:102-105` 加载新格式 dict；`:147` 改 `vision_np.knn1_predict(物品特征, X, y, classes)`
...
12. （可选）两 spec `mower_a` excludes 追加 `"sklearn", "scikit_image", "scipy"`
